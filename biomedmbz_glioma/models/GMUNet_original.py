...
class PositionAttentionModule(nn.Module):
    ...
    def __init__(self, d_model=512, kernel_size=3, H=7, W=7):
        ...
        self.pa = ScaledDotProductAttention(d_model, d_k=d_model, d_v=d_model, h=8)
...
class sc_attn(nn.Module):
    ...
    def __init__(self, d_model=512, kernel_size=3, H=7, W=7):
        ...
        self.channel_attention_module = ChannelAttentionModule(d_model=d_model, kernel_size=kernel_size, H=W, W=W)
...
class GraphCrossAttention(nn.Module):
    def __init__(self, in_dim):
        ...
        self.merge = nn.Conv2d(in_channels=in_dim * 2, out_channels=in_dim, kernel_size=1)

    @staticmethod
    def create_edge_index(h, w):
    ...
    def forward(self, x):
        res = x.clone().permute(0, 3, 1, 2)
        m_batchsize, height, width, c = x.shape
        edge_index = self.create_edge_index(height, width).to(x.device)
        y = x.view(m_batchsize * height * width, c)
        x = x.permute(0, 3, 1, 2)

        proj_query = self.query_conv(y, edge_index).view(m_batchsize, height, width, -1)
        proj_query_H = proj_query.permute(0, 2, 3, 1).contiguous().view(m_batchsize * width, -1, height).permute(0, 2,
                                                                                                                 1)
        proj_query_W = proj_query.permute(0, 1, 3, 2).contiguous().view(m_batchsize * height, -1, width).permute(0, 2,
                                                                                                                 1)

        proj_key = self.key_conv(y, edge_index).view(m_batchsize, height, width, -1)
        proj_key_H = proj_key.permute(0, 2, 3, 1).contiguous().view(m_batchsize * width, -1, height)
        proj_key_W = proj_key.permute(0, 1, 3, 2).contiguous().view(m_batchsize * height, -1, width)

        proj_value = self.value_conv(y, edge_index).view(m_batchsize, height, width, -1)
        proj_value_H = proj_value.permute(0, 2, 3, 1).contiguous().view(m_batchsize * width, -1, height)
        proj_value_W = proj_value.permute(0, 1, 3, 2).contiguous().view(m_batchsize * height, -1, width)
        ...
        y = self.gamma * (out_H + out_W) + x
        res = torch.concat([y, res], dim=1)
        res = self.merge(res).permute(0, 2, 3, 1)
...
class VSSLayer(nn.Module):
    def __init__(self, dim, depth, attn_drop=0., drop_path=0., norm_layer=nn.LayerNorm, downsample=None,
                 use_checkpoint=False, d_state=16, gatv2_args={}):
        super().__init__()
        self.dim = dim
...
class VSSM(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        for i_layer in range(self.num_layers):
            ...
            gc_attn = GraphCrossAttention(dims_decoder[i_layer])

            self.layers_up.append(layer)
            self.gc_up.append(gc_attn)
    ...
    def forward_features_up(self, x, skip_list):
        ...
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                ...
                x = layer_up(x + self.gc_up[inx - 1](skip_list[-inx]))
