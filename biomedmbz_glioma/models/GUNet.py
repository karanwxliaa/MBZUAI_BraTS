...
class GraphCrossAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        proj_query_D = proj_query.permute(0, 4, 3, 2, 1).contiguous().view(m_batchsize * height, -1, depth).permute(0, 2, 1)
        ...
        proj_key_D = proj_key.permute(0, 4, 3, 2, 1).contiguous().view(m_batchsize * height, -1, depth)
        ...
        proj_value_D = proj_value.permute(0, 4, 3, 2, 1).contiguous().view(m_batchsize * height, -1, depth)
        ...
        energy_D = torch.bmm(proj_query_D, proj_key_D).view(m_batchsize, height, width, depth)

        concate = self.softmax(torch.cat([energy_H, energy_W, energy_D], 3))

        att_H = concate[:, :, :, 0:height].permute(0, 2, 1, 3).contiguous().view(m_batchsize * width, height, height)
        att_W = concate[:, :, :, height:height + width].contiguous().view(m_batchsize * height, width, width)
        att_D = concate[:, :, :, height + width:height + width + depth].contiguous().view(m_batchsize * height, depth, depth)

        out_H = torch.bmm(proj_value_H, att_H.permute(0, 2, 1)).view(m_batchsize, width, -1, height).permute(0, 2, 3, 1)
        out_W = torch.bmm(proj_value_W, att_W.permute(0, 2, 1)).view(m_batchsize, height, -1, width).permute(0, 2, 1, 3)
        out_D = torch.bmm(proj_value_D, att_D.permute(0, 2, 1)).view(m_batchsize, height, -1, depth).permute(0, 2, 1, 3)
...
class MedNeXtGraphUNet(nn.Module):
